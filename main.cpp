...
void render()
{
    ...
    g_shader_program.set_view_matrix(g_view_matrix);
    DrawText(&g_shader_program, font_texture_id, MenuMsg, 0.5f, 0.1f, glm::vec3(3.0f, -1.0f, 0.0f));
    glClear(GL_COLOR_BUFFER_BIT);
    if (!GameOver && g_current_scene != g_main_menu) {
        DrawText(&g_shader_program, font_texture_id, Level1, 0.5f, 0.1f, glm::vec3(3.0f, -1.0f, 0.0f));
    ...
    if (GameOver && g_current_scene != g_main_menu) {
        if (PlayerWin) {
            DrawText(&g_shader_program, font_texture_id, WinMsg, 0.5f, 0.1f, glm::vec3(msg_spot, -1.0f, 0.0f));
        }
        else {
            DrawText(&g_shader_program, font_texture_id, LoseMsg, 0.5f, 0.1f, glm::vec3(msg_spot, -1.0f, 0.0f));
        }
    }
